...
@app.route('/view_date', methods=['GET', 'POST']) #date will be format of 20200310
def view_date():
    db = get_db()
    ...
    if request.method == 'POST':
        #output = request.form['date-select']
        #view(request.form['date-select'])
        #print (output)
        
    #return render_template('view_date.html', date_result=date_result)
	    entry_date = request.form['date-select']
	    ...
	    log_cur = db.execute('select food.name, food.protein, food.carbohydrates, food.fat, food.calories from log_date join food_date on food_date.log_date_id = log_date.id join food on food.id = food_date.food_id where log_date.entry_date = ?', [entry_date])
	    log_results = log_cur.fetchall()
	    ...
	    return render_template('day.html', entry_date=entry_date, pretty_date=pretty_date, \
	                            food_results=food_results, log_results=log_results, totals=totals)
    return render_template('view_date.html', date_result=date_result)
